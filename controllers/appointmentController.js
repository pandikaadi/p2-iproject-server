...
const createAppointment = async (req, res, next) => {
  try {
    ...
    if (+schedule > 5 || +schedule < 1) {
      throw { name: "scheduleValidationError" };
    } else {
  ...
  try {
    ...
    if (appointments) {
      ...
      let mailDetails = {
        from: "shavetiv8@gmail.com",
        to: req.currentUser.email,
        subject: "Success canceled your booking",
        text: `hi, ${req.currentUser.email}!
...
const editAppointment = async (req, res, next) => {
  try {
    const { lat, long, address, barberId, appointmentDate, schedule, price } =
      req.body;
    ...
    if (+schedule > 5 || +schedule < 1) {
      ...
      if (appointments) {
        ...
        let mailDetails = {
          from: "shavetiv8@gmail.com",
          to: req.currentUser.email,
          subject: "Success edited your booking",
          text: `hi, ${req.currentUser.email}!
